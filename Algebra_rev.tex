...
考虑对集合$S \coloneq \{(g,x)\mid gx =x\}$进行计数，如果原命题不成立，那么每一个$g_i$至少带来集合中的一个元素，且$\forall x,(1_G,x) \in S$，因此集合的大小大于$|G|$。然而对$x$计数，我们知道$|\text{Stab}(x)| = |G|/|X|$，且$\text{Stab}(x)$两两共轭，大小相等，可以得出$|S| = |G|$。这导出了矛盾。
